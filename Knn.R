...
training <- training[-1,-14]
test <- test[-1,-14]
predict_k3 <- kknn(formula=F21~., training, test, k=3,kernel ="triangular" )
fit <- fitted(predict_k3)
cm = as.matrix(table(Actual=test$F21,Fitted=fit))
sum(diag(cm))/length(test$F21)


predict_k5 <- kknn(formula=F21~., training, test, k=5,kernel ="triangular" )
fit <- fitted(predict_k5)
cm = as.matrix(table(Actual=test$F21,Fitted=fit))
sum(diag(cm))/length(test$F21)

predict_k10 <- kknn(formula=F21~., training[-1,-14], test[-1,-14], k=10,kernel ="triangular" )
